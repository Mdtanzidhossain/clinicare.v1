...
<?php
	...
	if(isset($_GET['view_id']) && !empty($_GET['view_id']))
	{
		...
		$stmt_edit = $DB_con->prepare('SELECT * FROM patient WHERE patient_id=:patient_id');
		$stmt_edit->execute(array(':patient_id'=>$view_id));
		$edit_row = $stmt_edit->fetch(PDO::FETCH_ASSOC);
		extract($edit_row);
		if(isset($_GET['action']) && $_GET['action'] =='finished'){
			$stmt_update = $DB_con->prepare("UPDATE orderdetails set order_status='Ordered_Finished' WHERE order_status = 'Ordered' and  patient_id=:patient_id");
			$stmt_update->execute(array(':patient_id'=>$view_id));
			if($stmt_update){
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <table class="display table table-bordered" id="example" cellspacing="0" width="100%">
			  ...
			  <?php
include("config.php");
	$stmt = $DB_con->prepare("SELECT * FROM orderdetails where patient_id='$patient_id' and order_status='Ordered'");
	$stmt->execute();
